...
int main() {
    ...
    // Physical properties
    const double emissivity_HP = 0.8;        // Wall emissivity [-]
    ...
    const double r_v = 0.00895;                                          // Vapor-liquid interface radius [m] (supposed 2 mm wick thickness not given)
    ...
    const double     accelerator = 2.0;              // Adaptive timestep multiplier (if too much, stability problems) [-]
    ...
    while (time_total < time_simulation) {
        ...
        for (pic = 0; pic < max_picard; pic++) {
            ...
            for (std::size_t i = 1; i < N - 1; ++i) {
                ...
                q_ow[i] = Q_ow[i] * (r_o * r_o - r_i * r_i) / (2 * r_o);
